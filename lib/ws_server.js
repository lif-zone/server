...
E.start = function(){
  ...
  wss.on('connection', (ws_src, req)=>{
    ...
    ws_src.on('message', (message, bin)=>{
      ...
      if (dst)
      {
        if (o.src!=ws_src.uuid)
          console.error('ws_server: fake src %s %s %o', src, ws_src.uuid, o);
        o.src = ws_src.uuid;
        ws_dst = ws_hash[dst];
        if (!ws_dst)
        {
      ...
      if (event=='join')
      {
        if (ws_hash[o.src]) // XXX: what to do if ws already have uuid
          console.error('ws_server: join already exists %s', o.src);
        ws_src.uuid = o.src;

...
E.start = function(){
  ...
  wss.on('connection', (ws_src, req)=>{
    ...
    ws_src.xxx_ip = (req.socket.remoteAddress||'').replace('::ffff:', '');
    ws_src.xxx_port = req.socket.remotePort;
    ws_src.json({event: 'connect', dst: ws_src.ws_id,
      data: {ws_id: ws_src.ws_id, ip: ws_src.xxx_ip, port: ws_src.xxx_port}});
    // XXX: need on 'json'
    ...
    ws_src.on('message', (message, bin)=>{
      ...
      if (event=='get_clients') // XXX temporary command for debug
      {
        ...
        wss.clients.forEach(ws=>{
          ...
          if (ws===ws_src)
            return;
          clients.push({ws_id: ws.ws_id, ip: ws.xxx_ip,
            port: ws.xxx_port});
        });
